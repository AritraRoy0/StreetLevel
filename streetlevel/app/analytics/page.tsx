...
import { MOCK_HISTORY, MOCK_STOCKS } from "@/lib/mock-data";
...
function AnalyticsPageContent() {
  ...
  const [selected, setSelected] = useState(MOCK_STOCKS[0].symbol);
  const [metric, setMetric] = useState<"price" | "momentum" | "risk">("price");
  const sectors = ["All sectors", ...Array.from(new Set(MOCK_STOCKS.map((stock) => stock.sector)))];
  const sectorStocks = selectedSector === "All sectors" ? MOCK_STOCKS : MOCK_STOCKS.filter((stock) => stock.sector === selectedSector);
  const stock = sectorStocks.find((item) => item.symbol === selected) ?? sectorStocks[0] ?? MOCK_STOCKS[0];
    ...
    <div className="min-h-screen bg-white text-black">
      ...
      <main className="mx-auto max-w-[1500px] px-5 py-10 sm:px-10 lg:px-16">
        ...
        <div className="space-y-12">
          ...
          <div className="grid gap-4 border-y border-black py-4 md:grid-cols-[1fr_1fr_auto] md:items-center"><label className="flex items-center gap-3"><span className="text-[10px] font-bold uppercase tracking-wider text-neutral-500">Sector</span><select value={selectedSector} onChange={(event) => { const next = event.target.value; const nextStocks = next === "All sectors" ? MOCK_STOCKS : MOCK_STOCKS.filter((item) => item.sector === next); setSelectedSector(next); setSelected(nextStocks[0]?.symbol ?? MOCK_STOCKS[0].symbol); }} className="min-w-0 bg-transparent font-mono text-sm font-bold text-black outline-none">{sectors.map((sector) => <option key={sector}>{sector}</option>)}</select></label><label className="flex items-center gap-3"><span className="text-[10px] font-bold uppercase tracking-wider text-neutral-500">Stock</span><select value={stock.symbol} onChange={(event) => setSelected(event.target.value)} className="min-w-0 bg-transparent font-mono text-sm font-bold text-black outline-none">{sectorStocks.map((item) => <option key={item.symbol}>{item.symbol}</option>)}</select></label><span className="text-xs text-neutral-500">{history.length} sessions / updated 14:32:08 ET</span></div>
